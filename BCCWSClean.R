...
in.BCCWS <- read.csv("Data/BCCWS.csv") # reads in back-up copy of database 
  ...
  event.BCCWS <- in.BCCWS %>% dplyr::select(SurveyAreaIdentifier, YearCollected, MonthCollected, DayCollected, DecimalLatitude, DecimalLongitude) %>% distinct()
  
  # retain columns that are needed for the analysis
  in.BCCWS <- in.BCCWS %>% dplyr::select(ProjectCode, SurveyAreaIdentifier, RouteIdentifier, SpeciesCode, ObservationCount,  wyear, YearCollected, MonthCollected, DayCollected, DurationInHours)
