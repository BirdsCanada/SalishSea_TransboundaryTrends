...
#will want to include DURATION in hours because this is highly variable and will want to use as a offset or covariate .
  ...
  event.BCCWS <- in.BCCWS %>% select(SurveyAreaIdentifier, YearCollected, MonthCollected, DayCollected) %>% unique()
  ...
  # recalculate duration in hours using the start and end times
  in.BCCWS$DurationInHours2 <- in.BCCWS$TimeObservationsEnded2 - in.BCCWS$TimeObservationsStarted2hours
  ...
  # and return index.data
   return(in.BCCWS)  # returns index.data for use in subsequent functions
  # write index.data to file
   write.csv(in.BCCWS, "BCCWS.clean.csv")
