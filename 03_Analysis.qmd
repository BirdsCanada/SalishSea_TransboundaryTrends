...
grid<-loc.xy %>% 
  ...
  distinct() %>% 
  mutate(Name = Name, #Rename your polygon name field
         Area = AreaSqKm,#Rename your area field 
         alpha_i = as.integer(factor(Name)) #create the alpha index for the analysis 
         ) 
...
poly <- st_read(polygon_shp) %>% st_make_valid()

#List available fields for user selection
cat("Polygon data fields:\n")
print(names(poly))

#Prompt user to select polygon ID and area fields
polygon_id_field <- #Enter the field name for polygon ID (e.g., 'WatershedID')
area_field <- #Enter the field name for area (or leave blank if none)

#Rename fields to 'Name' and 'Area', create area if missing
poly <- poly %>%
  rename(Name = !!polygon_id_field) %>%
  mutate(
    Area = if (area_field != "") !!sym(area_field) else 1
  )
...
plot <- ggplot() +
  geom_sf(data = Grid, aes(fill = Name), color = "black") +
  geom_sf(data = poly, size = 0.2, fill = NA) +
  geom_sf(data = xy, color = "black", size = 1) +
  theme_minimal()
...
ggsave(
  filename = file.path(plot_dir, "iCAR_Spatial_Plot.jpeg"),
  plot = plot,
  width = 8,
  height = 6,
  dpi = 300
...
# events <- ... # Load your events data here!
# events <- left_join(events, grid, by = "SurveyAreaIdentifier") %>% drop_na(Name)

# ---- END ----

cat("Processing complete! Fields standardized as 'Name' and 'Area'.\n")
