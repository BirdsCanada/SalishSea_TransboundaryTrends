...
**Alternatively**, you can set minimum data requirement. Here we select species that are detected by both surveys with a mean abundance per year greater than 5, and species that were detected in more than half of the survey years. These number can be adjusted to suit the users needs.
...
#Run the Species Selection script
source("SpeciesSelection.R")
#The output will be a list of species that the programs have in common and a sp.list that meet the minimum data requirements. 
...
When deriving population trends, we are often interesting in doing this for specific geographic areas of interest. These could be management units or areas of conservation concern. Here we will create a spatial object that represents the Salish Sea and its subregions: Country and Bird Conservation Regions (BCR). Other shapefiles can be used to define the spatial extent of interest. 
...
#Terrestrial and Marine BCR shapefiles can be found at the following link: https://www.birdscanada.org/bird-science/nabci-bird-conservation-regions

#Download and save to your Data directory in your working directory
