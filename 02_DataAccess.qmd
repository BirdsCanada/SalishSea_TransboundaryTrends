...
During this process some species are combined following advise from the survey coordinators. These are species that are typically difficult to tell apart and are often misidentified. Other non-target species are also removed. Any species detected less than 10 times over all years is also considered rare and removed. 

-   "Large Gulls" = gull (large) + WEGU + GWGU hybrid + Glaucous-winger + Western + Herring + Glaucous + Iceland (Thayer's) + California Gull
...
To manually create your list, you can use the species name. For example, to include the species: Surf Scoter (SUSC), Long-tailed Duck (LTDU), and Buffelhead (BUFF), you would create a list as follows:
...
#There are 53 species in common between BCCWS and PSSS
