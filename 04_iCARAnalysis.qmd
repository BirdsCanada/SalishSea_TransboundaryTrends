...
ggplot() +
  ...
  geom_sf(data = xy, color = "black", size = 2) +
  theme_minimal()
...
#Finally, set up the grid key which you will use later to align alpha_id back with your polygon names and/or ID. 

#may not needs this afterall
grid_key<-unique(events[, c("Name", "alpha_i")])
grid_name<-Grid %>% select(Name)
grid_key <-left_join(grid_key, grid_name, by=c("Name")) #include the geometry
